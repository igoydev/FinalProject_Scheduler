...
beginning:
    ...
    cout << setw(4) << left << "No " << setw(12) << left << "|Date"
         << "|Start" <<setw(6) << left << "|End" << setw(17) << "|Activity"
         << "|" << endl;
    for (int i = 0; i < date.size(); i++)
    {
        cout << setw(3) << right << i + 1 << ".|" << setw(11) << left << date[i] << "|" << setw(5) << left << hours[i] << "|" << setw(5) << left << hoursEnd[i] << "|" << setw(16) << activity[i] << "|" << endl;
    }
...
invalid:
    ...
    {
        name.erase(name.begin() + choice);
        date.erase(date.begin() + choice);
        hours.erase(hours.begin() + choice);
        hoursEnd.erase(hoursEnd.begin() + choice);
...
void add_schedule()
{
    string new_name, new_date, new_hours, new_hoursEnd, new_activity;
    header();
    cin.ignore();
    cout << "Name : ";
    ...
    cout << new_date << " Time Start: ";
    getline(cin, new_hours);
    cout << new_date << " Time End: ";
    getline(cin, new_hoursEnd);
    ...
    hoursEnd.push_back(stof(new_hoursEnd));
    activity.push_back(new_activity);
    ofstream myFile("Schedule.txt", ios::app);
    myFile << "\n"
           << new_name << "," <<new_date << "," << new_hours << "," << new_hoursEnd << "," << new_activity;
    myFile.close();
    dashboard();
}
